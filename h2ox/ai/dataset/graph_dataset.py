...
class GraphDataset(Dataset):
    ...
    def engineer_arrays(self, data: xr.Dataset):
            ...
            if self.norm_difference:
                for var in self.target_var:
                    self.augment_dict["normalise"][var] = {
                        "max": data[var]
                        .groupby("global_sites")
                        .map(lambda arr: arr.max()),
                        "min": data[var]
                        .groupby("global_sites")
                        .map(lambda arr: arr.min()),
                    }
                    data[var] = data[var].groupby("global_sites").map(norm_func_nv)
        ...
        for ii, idx in enumerate(idxs):
            ...
            if not (
                np.isnan(self.historic[ii, ...]).any()
                + np.isnan(self.forecast[ii, ...]).any()
                + np.isnan(self.forecast[ii, ...]).any()
                + np.isnan(self.targets[ii, ...]).any()
            ):
                ...
                if isinstance(idx, tuple):
                    self.sample_lookup[data_ii] = dict(zip(self.metadata_columns, idx))
                else:
