    ...
    for epoch in range(epochs):
        ...
        for ii, data in enumerate(pbar):
            ...
            (
                ...
                criterion=loss_fn,
                sample_nbr=3,
                complexity_cost_weight=1.0 / data["x_d"].shape[0] / 500000,
            )
...
def validate(
    ...
    for data in pbar:
        # move to device
        X = {
            kk: vv.to(device) for kk, vv in data.items() if kk in ["x_d", "x_f", "x_ff"]
        }
        data["y"] = data["y"].to(device)
        ...
        means, stds, ci_upper, ci_lower = mc_sample(model, X, N_samples=50)
...
def test(
    ...
    for data in tqdm(test_dl, "Running Evaluation"):
        ...
        means, stds, ci_upper, ci_lower = mc_sample_paths(model, X, N_samples=50)
