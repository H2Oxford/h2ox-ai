...
def whole_visualise(model, whole_loader, device, savepath):
    ...
    results_df = pd.DataFrame(
        ...
        data=np.concatenate(Y_hat_results) * whole_loader.dataset.Y_std
        + whole_loader.dataset.Y_mean,
        columns=[
            timedelta(days=ii)
            for ii in [1] + list(range(5, whole_loader.dataset.targets_forecast, 5))
        ],
    )
    ...
    for ii_a, forecast_offset in enumerate([5, 30, 60, 90]):
        ...
        axs[ii_a].plot(
            ...
            whole_loader.dataset.df.loc[results_df.index, "Y"]
            * whole_loader.dataset.Y_std
            + whole_loader.dataset.Y_mean,
            c="k",
            linestyle="solid",
            lw=2,
        ...
        axs[ii_a].plot(
            (results_df.index.values + np.timedelta64(forecast_offset, "D")).astype(
                np.int64
            )
            // 10 ** 9,
...
def eval_r2(model, test_loader, device):
    ...
    Y_results = (
        np.concatenate(Y_results) * test_loader.dataset.Y_std
        + test_loader.dataset.Y_mean
    )
    Y_hat_results = (
        np.concatenate(Y_hat_results) * test_loader.dataset.Y_std
        + test_loader.dataset.Y_mean
    )
...
def eval_visualise(model, val_loader, device, savepath):
    ...
    results_df = pd.DataFrame(
        ...
        data=np.concatenate(Y_hat_results) * val_loader.dataset.Y_std
        + val_loader.dataset.Y_mean,
        columns=[
            timedelta(days=ii)
            for ii in [1] + list(range(5, val_loader.dataset.targets_forecast, 5))
        ],
    )

    results_df = results_df.sort_index()

    val_starts = val_loader.dataset.df.loc[
        (val_loader.dataset.df["segment"] == "val")
        & (
            val_loader.dataset.df["segment"]
            != val_loader.dataset.df["segment"].shift(1)
        ),
        "segment",
    ].index.values
    val_stops = val_loader.dataset.df.loc[
        (val_loader.dataset.df["segment"] == "val")
        & (
            val_loader.dataset.df["segment"]
            != val_loader.dataset.df["segment"].shift(-1)
    ...
    for ii_a in range(axs.shape[0]):
        ...
        axs[ii_a].plot(
            results_df.loc[
                (results_df.index >= start_dt) & (results_df.index < end_dt)
            ].index.values.astype(np.int64)
            // 10 ** 9,
            val_loader.dataset.df.loc[
                results_df.loc[
                    (results_df.index >= start_dt) & (results_df.index < end_dt)
                ].index,
                "Y",
        ...
        axs[ii_a].plot(
            val_loader.dataset.df.loc[
                (val_loader.dataset.df.index >= end_dt)
                & (val_loader.dataset.df.index < (end_dt + np.timedelta64(60, "D")))
            ].index.values.astype(np.int64)
            // 10 ** 9,
            val_loader.dataset.df.loc[
                (val_loader.dataset.df.index >= end_dt)
        ...
        start_ts = (start_dt - np.datetime64("1970-01-01T00:00:00Z")) / np.timedelta64(
            1, "s"
        )
        end_ts = (
            end_dt + np.timedelta64(60, "D") - np.datetime64("1970-01-01T00:00:00Z")
        ) / np.timedelta64(1, "s")
...
def main(PARAMS):
    ...
    model = W2W_DNN(input_dim=_X.shape[0], output_dim=_Y.shape[0])
    ...
    train(
        ...
        PARAMS,
        device=device,
        savepath=os.path.join(root, PARAMS["SITE"] + "_DNN_break_timeseries.png"),
        verbose=True,
    )
