...
def main(PARAMS):
    ...
    trn_dataset = w2w_lstm_loader(
        csv_data_path=os.path.join(root, 'wave2web_data','refresh_09-27',f'{PARAMS["SITE"]}_3zscore.csv'), 
        horizon_days=90, 
        lead_days=60, 
        segment=['trn'],
        site = PARAMS["SITE"],
    ...
    torch.save(model.state_dict(), os.path.join(os.getcwd(),'w2w_results','refresh_09-27',f'{whole_loader.dataset.site}_weights.pth'))
...
def perturb(pert_amt, model, device):
    ...
    pert_dataset = w2w_lstm_loader(
        csv_data_path=os.path.join(os.getcwd(), 'wave2web_data','refresh_09-27',f'{PARAMS["SITE"]}_3zscore.csv'), 
        horizon_days=90, 
        lead_days=60, 
        segment=['trn','val','test','deploy'],
        perturb=pert_amt,
    ...
    results_df = pd.DataFrame(
        ...
        data=data,
        columns = [timedelta(days=ii) for ii in list(range(1,pert_loader.dataset.horizon_days+1))]
    )
    
    results_df = results_df.sort_index()

    results_df.to_csv(os.path.join(os.getcwd(),'w2w_results','refresh_09-27',pert_loader.dataset.site+f'p{pert_amt}_full_result.csv'))   
...
def test_output(model,test_loader, device):
    ...
    json.dump(rscores, open(os.path.join(os.getcwd(),'w2w_results','refresh_09-27',test_loader.dataset.site+'_r2score.json'),'w'))
...
def whole_visualise_and_output(model,whole_loader, device):
    ...
    results_df.to_csv(os.path.join(os.getcwd(),'w2w_results','refresh_09-27',whole_loader.dataset.site+'_full_result.csv'))   
    
    results_df = results_df.unstack().reset_index()
    
    results_df['prediction_dt'] = results_df['level_0']+results_df['level_1'] # date + timedelta
    
    summary_df = results_df.loc[results_df['level_0'].isin([timedelta(days=ii) for ii in [30,60,90]])]
    
    summary_df = summary_df.set_index(['level_0','prediction_dt']).drop(columns='level_1').unstack('level_0')
